// credit: mikkokko
// source: https://www.youtube.com/watch?v=-SKjuLDYQcI
...
public:
	...
	~CAudioXAudio2(void);
	bool		Init(const audio_device_init_params_t& params, const audio_device_description_t& desc);
...
typedef HRESULT(STDAPICALLTYPE* XAudio2Create_t)(IXAudio2**, UINT32, XAUDIO2_PROCESSOR);

static bool InitXAudio()
{
	if (!g_pXAudio2)
	{
		HMODULE hXAudio2 = LoadLibrary(XAUDIO2_DLL);
		if (!hXAudio2)
			return false;

		XAudio2Create_t pXAudio2Create = (XAudio2Create_t)GetProcAddress(hXAudio2, "XAudio2Create");
		if (!pXAudio2Create)
			return false;
		...
		if (FAILED(hr = pXAudio2Create(&g_pXAudio2, XAUDIO2_DEBUG_ENGINE, XAUDIO2_DEFAULT_PROCESSOR)))
#endif
			if (FAILED(hr = pXAudio2Create(&g_pXAudio2, 0, XAUDIO2_DEFAULT_PROCESSOR)))
				return false;
...
int Audio_EnumerateXAudio2Devices(audio_device_description_t* pDeviceListOut, int nListCount)
{
	...
	HRESULT hr = CoCreateInstance(__uuidof(MMDeviceEnumerator), NULL, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(pDeviceEnumerator.GetAddressOf()));
	Assert(hr == S_OK);
	if (hr != S_OK)
		return 0;

	Microsoft::WRL::ComPtr<IMMDeviceCollection> pDeviceCollection;
	hr = pDeviceEnumerator->EnumAudioEndpoints(eRender, DEVICE_STATE_ACTIVE, pDeviceCollection.GetAddressOf());
	if (hr != S_OK)
		return 0;

	UINT nDeviceCountWindows = 0;
	hr = pDeviceCollection->GetCount(&nDeviceCountWindows);
	if (hr != S_OK)
		return 0;
	...
	Microsoft::WRL::ComPtr<IMMDevice> pDefaultDevice;
	hr = pDeviceEnumerator->GetDefaultAudioEndpoint(eRender, eConsole, pDefaultDevice.GetAddressOf());
	if (hr == S_OK)
	{
	...
	for (int i = 0; i < nIterateCount; i++)
	{
		...
		hr = pDeviceCollection->Item(i, pDevice.GetAddressOf());
		if (hr != S_OK)
			return 0;
		...
		hr = pDevice->OpenPropertyStore(STGM_READ, pPropertyStore.GetAddressOf());
		if (hr != S_OK)
			return 0;
		...
		if (hr != S_OK || FriendlyName.vt != VT_LPWSTR)
			return 0;

		PROPVARIANT DeviceFormat;
		PropVariantInit(&DeviceFormat);
		hr = pPropertyStore->GetValue(PKEY_AudioEngine_DeviceFormat, &DeviceFormat);
		if (hr != S_OK || DeviceFormat.vt != VT_BLOB)
			return 0;

		PWAVEFORMATEX pWaveFormat = (PWAVEFORMATEX)DeviceFormat.blob.pBlobData;

		V_wcscpy_safe(pDeviceListOut[i].m_deviceName, DeviceID);
		V_wcstostr(FriendlyName.pwszVal, -1, pDeviceListOut[i].m_friendlyName, sizeof(pDeviceListOut[i].m_friendlyName));
		pDeviceListOut[i].m_nChannelCount = pWaveFormat->nChannels;
		...
		if (DefaultDeviceID && !wcscmp(DefaultDeviceID, DeviceID))
		{
			pDeviceListOut[i].m_bIsDefault = true;
			bWroteDefault = true;
		}
		if (nMaxChannelDevice < 0 || pWaveFormat->nChannels > pDeviceListOut[nMaxChannelDevice].m_nChannelCount)
		{
			nMaxChannelDevice = i;
		}
	}
...
// dumb as fuck
static bool GetDeviceCount(UINT* pDeviceCount)
{
	Microsoft::WRL::ComPtr<IMMDeviceEnumerator> pDeviceEnumerator;
	HRESULT hr = CoCreateInstance(__uuidof(MMDeviceEnumerator), NULL, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(pDeviceEnumerator.GetAddressOf()));
	Assert(hr == S_OK);
	...
	hr = pDeviceCollection->GetCount(pDeviceCount);
	if (hr != S_OK)
		return false;
...
IAudioDevice2* Audio_CreateXAudio2Device(const audio_device_init_params_t& params)
{
	...
	if (GetDeviceCount(&nDeviceCountWindows))
	{
	...
	if (pDevice->Init(params, desc[nPreferredDevice]))
		return pDevice;
...
bool CAudioXAudio2::Init(const audio_device_init_params_t& params, const audio_device_description_t& desc)
{
	HRESULT hr;
	// NOTE: sample rate was XAUDIO2_DEFAULT_SAMPLERATE
	if (FAILED(hr = g_pXAudio2->CreateMasteringVoice(&m_pMasterVoice, XAUDIO2_DEFAULT_CHANNELS, XAUDIO2_DEFAULT_SAMPLERATE, 0, desc.m_deviceName, NULL)))
		return false;

	V_wcscpy_safe(m_deviceID, desc.m_deviceName);
	V_strcpy_safe(m_displayName, desc.m_friendlyName);
